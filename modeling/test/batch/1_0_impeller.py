...
from pyKrigine.krige import krige
...
class ParamsGroup:
    def __init__(self, nameList, domainSamples):
        self.nameList = nameList
        self.propNumbers = len(self.nameList)

        self.domainSamples = domainSamples
        self.sampleSize = len(self.domainSamples)

        self.currentIndex = 0
        self.indexList = list(range(0, self.sampleSize, 1))

    def setAndBindValue(self, index):
        self.currentIndex = index
        bindPropertiesWithoutObj(self.nameList, [self.domainSamples[self.currentIndex]]*self.propNumbers)
...
# For GA initial population generation
thicknessGroup = ParamsGroup(["ThicknessShroud", "ThicknessHub", "ThicknessLEShroud", "ThicknessTEShroud", "ThicknessLEHub", "ThicknessTEHub", "ThicknessLEAve", "ThicknessTEAve"], [20, 50])
diameterGroup = ParamsGroup(["ds"], [60, 90])

paramsGroupList = [thicknessGroup, diameterGroup]
iterIndexList = list(it.product(*list(map(lambda x: x.indexList, paramsGroupList))))

for iterations in iterIndexList:
    for i in range(0, len(iterations), 1):
        paramsGroupList[i].setAndBindValue(iterations[i])
    ...
    if Bla.FullDomainCFD == False and Bla.PeriodicDomainCFD == False:
        ...
        Part.export(__objs__,path_gui+u"/impeller"+str(iterIndexList.index(iterations)+1)+".step")
        del __objs__
    else:
        __objs__=[]
