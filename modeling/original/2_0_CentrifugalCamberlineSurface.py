...
class ModelOfBlade3D:
	...
	def execute (self,obj):### file:///C:/Users/newton/Desktop/RadialTurboMacrosFreeCAD-main/3_CentrifugalImpeller3D.py
		import  Part, FreeCAD
		import numpy as np
		import matplotlib.pyplot as plt

		N=obj.N### obj.addProperty 로 입력한 N 값을 받아옴 (1000)
		Mer=FreeCAD.ActiveDocument.getObject("Meridional")###(Mer으로 Merididonal object의 property들을 사용할 수 있게 정의)
		shroudEdge = Mer.Shape.Edges[3]
		shroudEdgeDiscret = shroudEdge.discretize(Number = N)
		shroudEdgeDiscret.append(shroudEdge.lastVertex().Point)### ??? 이거 꼭 필요한 코드인가? - 마지막 꼭지점 값 똑같은게 하나 더 추가되네

		hubEdge = Mer.Shape.Edges[7]
		hubEdgeDiscret = hubEdge.discretize(Number = N)
		hubEdgeDiscret.append(hubEdge.lastVertex().Point)

		N = len(shroudEdgeDiscret) ### N = list의 길이 만큼으로 다시 받아옴 (1000 -> 1001 으로 바뀜) but why?
...
##### THIS IS FUNCTION FOR CREATION OF A STREAMLINES FROM MERIDIONAL PLAN ###############################################
		def streamlineBlade (AngleBeta1, AngleBetaPoint1, AngleBetaPoint2, AngleBeta2, DiscretEdgeOfMeridional):
			...
			betaCurveDiscret = []
			for i in range (1, (N+1), 1):### 1에서부터 N까지 1개씩 받아오기
...
######################################THIS FUNCTION CREATES A CHART OF THE PARAMETERS STREAMLINE#########################
		def chartBetaTheta (betai, dThetaSum, AnglesChart):
			if AnglesChart ==True:
				...
				plt.show()
#####################################THE END OF THIS FUNCTION ###########################################################
### 얘들은 execute 함수에서 돌아가는 애들 -> execute 내부 함수인 streamlineBlade를 이용했음
...
#################################################################################################################
		...
		if Mer.CylinricalBlades == False:### Shroud 랑 Hub는 execute 초반부분에 이미 해놨음 -> 이거 그대로 average line에서 다시 하는거임
